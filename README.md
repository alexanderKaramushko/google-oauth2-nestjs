...
1. Создается гард JwtGuard, который проверяет id-токен через OAuth2Client
2. Защищенный роут прогоняется через JwtGuard
...
* [ ] Добавить возможность редиректа в приложение через state, которое запросило авторизацию  *
